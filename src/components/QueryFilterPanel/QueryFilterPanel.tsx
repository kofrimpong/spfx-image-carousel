...
export class QueryFilterPanel extends React.Component<IQueryFilterPanelProps, IQueryFilterPanelState> {
    ...
    private onDeleteFilterClick(filter: IQueryFilter) {
        const remainingFilters = this.state.filters.filter((i) => { return i.index !== filter.index; });
        this.setState({ ...this.state, ...{ filters: remainingFilters } });
    }
    ...
    public render() {
        ...
        const filters = this.state.filters.map((filter, index) =>
            <div className={styles.queryFilterPanelItem} key={index}>
                <Stack tokens={{ childrenGap: 5 }} horizontal={true}>
                    <Stack.Item grow={1}>
                <QueryFilter fields={this.state.fields}
                    ...
                    <Stack.Item>
                        {index !== 0  && <CommandButton iconProps={{ iconName: 'Trash' }} onClick={() => this.onDeleteFilterClick(filter)} style={{ marginTop: 20 }} />}
                    </Stack.Item>
                </Stack>
            </div>
        );
