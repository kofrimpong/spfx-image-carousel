...
import { ITag, IBasePickerSuggestionsProps, NormalPeoplePicker, TagPicker } from 'office-ui-fabric-react/lib/Pickers';
...
export class QueryFilter extends React.Component<IQueryFilterProps, IQueryFilterState> {
    ...
    private onDatePickerFormat(date: Date): string {
        return format(date, this.props.strings.datePickerFormat);
    }
    ...
    private onDatePickerParse(dateStr: string): Date {
        try {
            return parse(dateStr, this.props.strings.datePickerFormat,new Date())
        }
        catch (error) {
            return new Date()
        }
