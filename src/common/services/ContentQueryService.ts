...
import { IQuerySettings }                                       from '../constants/IQuerySettings';
import { CamlQueryHelper }                                      from '../helpers/CamlQueryHelper';
import { ListService, IListTitle }                              from './ListService';
import { SearchService }                                        from './SearchService';
import { IContentQueryContext } from './IContentQueryContext';
import { IQueryFilterField } from '../../components/QueryFilter/IQueryFilterField';
import { QueryFilterFieldType } from '../../components/QueryFilter/QueryFilterFieldType';
...
export class ContentQueryService {
    ...
    private viewFields: IDropdownOption[] | IChecklistItem[];
     ...
     **************************************************************************************************/
    public getContentQueryContext(querySettings: IQuerySettings, callTimeStamp: number): Promise<IContentQueryContext> {
        ...
        return new Promise<IContentQueryContext>((resolve,reject) => {

            // Initializes the base template context
            let templateContext : IContentQueryContext = {
                pageContext: this.context.pageContext,
                webUrl: querySettings.webUrl,
                legend:querySettings.legend,
            ...
            this.listService.getListItemsByQuery(querySettings.webUrl, querySettings.listId, query)
                .then((data: any) => {
                    ...
                    let normalizedResults = this.normalizeQueryResults(data.value)
     ...
     **************************************************************************************************/
    public getPictureListTitleOptions(webUrl: string): Promise<IDropdownOption[]> {
        ...
        return new Promise<IDropdownOption[]>((resolve, reject) => {
            this.listService.getPictureLibraryTitlesFromWeb(webUrl).then((listTitles:IListTitle[]) => {
     ...
     * @param listId : The id of the list from which the field must be loaded from
     **************************************************************************************************/
    public getViewFieldsDropdownOptionsItems(webUrl: string, listId: string):Promise<IDropdownOption[]> {
        ...
        // Resolves the already loaded data if available
        if(this.viewFields) {
            return Promise.resolve(<IDropdownOption[]>this.viewFields);
        }
        ...
        return new Promise<IDropdownOption[]>((resolve, reject) => {
            this.listService.getListFields(webUrl, listId, ['InternalName', 'Title'], 'Title').then((data:any) => {
                let fields:any[] = data.value;
                let options:IDropdownOption[] = [ { key: "", text: strings.LegendFieldPlaceholder } ];
                let items:IDropdownOption[] = fields.map((field) => { return {
                    key: field.InternalName,
                    text: Text.format("{0} \{\{{1}\}\}", field.Title, field.InternalName)
                }; });
                options = options.concat(items);
                this.viewFields = options;
                resolve(options);
            })
            .catch((error) => {
                reject(this.getErrorMessage(webUrl, error));
    ...
    public getViewFieldsChecklistItems(webUrl: string, listId: string):Promise<IChecklistItem[]> {
        ...
        // Resolves the already loaded data if available
        if(this.viewFields) {
            return Promise.resolve(<IChecklistItem[]>this.viewFields);
        }
     ...
     **************************************************************************************************/
    private normalizeQueryResults(results: any[]): any[] {
        ...
        for(let result of results) {
            normalizedResults.push(result.FieldValuesAsText);
        }
