...
import { QueryFilterOperator } from '../../components/QueryFilter/QueryFilterOperator';
import { QueryFilterJoin } from '../../components/QueryFilter/QueryFilterJoin';
    ...
    private static formatDateExpressionFilterValue(filterValue: string): string {
        ...
        if (results != null) {
            for (let result of results) {
                ...
                let formatDate = format(dt,"yyyy-MM-dd'T'hh:mm:ss'Z'");
                filterValue = filterValue.replace(result, formatDate);
            }
        }

        // Replaces any "[Today]" expression by it's actual value
        let formattedDate = format(new Date(),"yyyy-MM-dd'T'hh:mm:ss'Z'");
